...
class EnsureCorrectAPIHeadersTest extends TestCase
{
    ...
    public function it_aborts_post_request_if_content_type_header_does_not_adhere_to_json_api_spec()
    {
        ...
        $response = $middleware->handle($request, function($request){
            $this->fail('Did not abort request because of infalid Content-Type header');
        });
     ...
     * @test
     */
    public function it_accept_post_request_if_content_type_header_adheres_to_json_api_spec()
    {
        $request = Request::create('/test','POST');
        $request->headers->set('accept', 'application/vnd.api+json');
    ...
    // public function it_ensure_that_a_content_type_header_adhering_to_json_api_spec_is_on_response()
    // {
    //     $request = Request::create('/test', 'GET');
    //     $request->headers->set('accept','application/vnd.api+json');
    //     $request->headers->set('content-type', 'application/vnd.api+json');
    ...
    //     $this->assertEquals(200, $response->status);
    //     $this->assertEquals('application/vnd.api+json', 
